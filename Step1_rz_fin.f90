...
! Constants
    ...
    real(8), parameter :: alpha_0 = 1.374E-7, lambda_0 = 0.50 !alpha_0 = 1.524e-4, lambda_0 = 0.632 !alpha_0 = 1.374E-7, lambda_0 = 0.50         ! ROW = 1050, CP = 3465, lambda = 0.50, (Thermophysical properties)
    real(8), parameter :: h = 0.010                                   ! Heat transfer coefficient
    real(8), parameter :: Ts = 37.0, Tf = 20.0                        ! Human core temperature
    real(8), parameter :: Heatsource = 25                                        ! Heat flux
    ...
    real(8) :: dr, dz, dt,aa,bb, q,Area
    real(8) :: dT_dr2, dT_dr, dT_dz2
    character(len=30) :: filename
    
    ! Get user inputs
    ! Enter the number of grid points : "
    integer :: nr = 501, nz = 1001, nt = 1000, z1, z2
...
!**************Boundary conditions of z direction*****************
        ...
        b( 1 , : ) = bb
        c( 1 , : ) = 1-bb
        d( 1 , : ) = (-2*bb*dz*h*(T(1,:)-Tf))/lambda_0+Sz(2,:)
...
!        If (MOD (n,1000).eq. 1) then    
        If (1 .eq. 1) then    
            write(filename, '(A, I8.8, A)') 'Temperature_', n, '.csv'
    ...
    write(filename, '(A, I8.8, A)') 'Temperature_', nt, '.csv'
    open(unit=11, file=filename, status="replace", action="write")
        Do i = 1, nz
            write(11,*) (T(i,j), j=1,nr)
        End do
