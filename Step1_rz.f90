...
! Constants
	...
	real(8), allocatable, dimension(:) :: ddd,dddd,aaa,bbb,ccc,aaaa,bbbb,cccc
	real(8) :: dr, dz, dt,aa,bb


	! Get user inputs
	! Enter the number og grid points : "
	integer :: nr = 51, nz = 101, nt = 2
	...
	allocate(T(nz,nr,nt+1), S(nz,nr), Sz(nz,nr), deltaT(nz,nr), a(nz,nr), b(nz,nr), c(nz,nr), d(nz,nr), r(nz,nr), z(nz,nr),  ddd(nz), dddd(nr), aaa(nz), bbb(nz), ccc(nz), aaaa(nr), bbbb(nr), cccc(nr) )
...
! Solve S(i,j)
	  ...
	  Do j = 2, nr-1
		Do i = 2, nz-1
		   S(i,j) = (alpha_0*dt/2)*((T(i,j-1,n)-2*T(i,j,n)+T(i,j+1,n))/dr**2+(1/r(i,j))*((T(i,j+1,n)-T(i,j-1,n))/2/dr)+(T(i-1,j,n)-2*T(i,j,n)+T(i+1,j,n))/dz**2)
		End Do
	  End Do
...
! Solve the tridiagonal system using the Thomas Algorithm
	  Do j = 1,nr
		...
		bbb = b(:,j)
		ccc = c(:,j)
	  Call solve_tridiag_r(aaa,bbb,ccc,ddd,Sz(:,j),Nr)
	  end do
...
open(unit=11, file="temperature_rz.csv", status="replace", action="write")
Do i = 1, nz
